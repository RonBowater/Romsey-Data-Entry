...
(function(root, factory) {
  ...
  Backform = root.Backform = {
    ...
    formClassName: "backform",
    groupClassName: "form-group",
    controlLabelClassName: "control-label rjb-form-adjust1",
    controlsClassName: "rjb-form-adjust2",
  ...
  */
  ...
  var InputControl = Backform.InputControl = Control.extend({
    ...
    template: _.template([
      '<label class="<%=Backform.controlLabelClassName%>"><%=label%></label>',
      '<div class="<%=Backform.controlsClassName%>">',
      '  <input autocomplet="off" type="<%=type%>" class="<%=Backform.controlClassName%> <%=extraClasses.join(\' \')%>" name="<%=name%>" maxlength="<%=maxlength%>" value="<%-value%>" placeholder="<%-placeholder%>" <%=disabled ? "disabled" : ""%> <%=required ? "required" : ""%> />',
      '  <% if (helpMessage && helpMessage.length) { %>',
      '    <span class="<%=Backform.helpMessageClassName%>"><%=helpMessage%></span>',
      '  <% } %>',
      '</div>'
  ...
  var BooleanControl = Backform.BooleanControl = InputControl.extend({
    ...
    template: _.template([
      ...
      '  <div class="checkbox">',
      '    <label>',
      '      <input autocomplete="off" type="<%=type%>" class="<%=extraClasses.join(\' \')%>" name="<%=name%>" <%=value ? "checked=\'checked\'" : ""%> <%=disabled ? "disabled" : ""%> <%=required ? "required" : ""%> /> <%=label%>',
      '    </label>',
      '  </div>',
      '</div>'
    ].join("\n")),
  ...
  var RadioControl = Backform.RadioControl = InputControl.extend({
    ...
    template: _.template([
      ...
      '      <input autocomplete="off" type="<%=type%>" class="<%=extraClasses.join(\' \')%>" name="<%=name%>" value="<%-formatter.fromRaw(option.value)%>" <%=rawValue == option.value ? "checked=\'checked\'" : ""%> <%=disabled ? "disabled" : ""%> <%=required ? "required" : ""%> /> <%-option.label%>',
      '    </label>',
      '  <% } %>',
      '  <% if (helpMessage && helpMessage.length) { %>',
      '    <span class="<%=Backform.helpMessageClassName%>"><%=helpMessage%></span>',
  ...
  var DatepickerControl = Backform.DatepickerControl = InputControl.extend({
    ...
    render: function() {
      InputControl.prototype.render.apply(this, arguments);
       ...
       this.$el.find("input").datepicker({dateFormat: 'dd M yy'});
      return this;
    }
  });
  
  var TimepickerControl = Backform.TimepickerControl = Control.extend(
  {
    defaults: 
    {
      label: "",
    ...
    template: _.template([
      '<label class="<%=Backform.controlLabelClassName%>"><%=label%></label>',
      '<div class="<%=Backform.controlsClassName%> clockpicker">',
      
    
      	'<input autocomplete="off" id="time" type="text" name="<%=name%>" class="form-control">',
  ...
  var MxclockpickerControl = Backform.MxclockpickerControl = InputControl.extend(
  {
	  defaults: 
	  { // Extra options that can be passed in the field defition.
		type: "text"
	  },
	  template: _.template(
	  [
      	'<div class="input-group clockpicker">',
    	'<input autocomplete="off" type="text" class="form-control" value="09:30">',
